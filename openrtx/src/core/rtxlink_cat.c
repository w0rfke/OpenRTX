...
static void catProtocolHandler(const uint8_t *data, size_t len)
{
    uint8_t reply[32];
    size_t  rLen = 0;
    ...
    {
        case CAT_FRAME_GET:
            rLen = catCommandGet(args, len, reply);
            break;

        case CAT_FRAME_SET:
            rLen = catCommandSet(args, len, reply);
            break;
    ...
    rtxlink_send(RTXLINK_FRAME_CAT, reply, rLen);
}
