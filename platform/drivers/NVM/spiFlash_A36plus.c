...
{
		// Wait until the TXE flag is set (Transmit buffer empty)
		while ((SPI2->SR & SPI_SR_TXE) == 0)
			; 

		// Write data to the SPI data register (Transmit)
		SPI2->DR = val; 

		// Wait until the BSY flag is cleared (SPI not busy)
		while ((SPI2->SR & SPI_SR_BSY) != 0)
			; 

		// Wait until the RXNE flag is set (Receive buffer not empty)
		while ((SPI2->SR & SPI_SR_RXNE) == 0)
			; 

		// Read received data from the SPI data register
			return SPI2->DR;
...
void spiFlash_init()
{
		...
		//Already done in main.c (OUTSIDE openrtx)
